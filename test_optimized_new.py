...
class NetMnistFC(nn.Module):
    def __init__(self):
        super(NetMnistFC, self).__init__()
        # Input layer
...
class NetCifar10(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # Convolutional layers following the VGG11 structure
        self.features = nn.Sequential(
            ...
            nn.Conv2d(256, 512, kernel_size=3, padding=1), # Conv5
            nn.ReLU(inplace=True),
            nn.Conv2d(512, 512, kernel_size=3, padding=1), # Conv6
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),         # Pool4
            
            nn.Conv2d(512, 512, kernel_size=3, padding=1), # Conv7
            nn.ReLU(inplace=True),
        ...
        self.classifier = nn.Sequential(
            nn.Linear(512 * 1 * 1, 4096), # Flattened feature map size is 512 * 1 * 1
            nn.ReLU(inplace=True),
            nn.Dropout(),
            nn.Linear(4096, 4096),
            nn.ReLU(inplace=True),
...
def train_nets(nets, n_epocas, train_dataset, idx, device):
    ...
    for i, net in enumerate(nets):
        ...
        #print(idx[i])
        part_loader = DataLoader(Subset(train_dataset, idx[i]), batch_size=64, shuffle=True)
...
def train_random(nets, n_redes, n_epochs, train_dataset, idx, device):
    ...
    for i in range(n_redes):
        ...
        # Seleciona um par aleatório (evitando a própria rede)
        j = random.choice([x for x in range(n_redes) if x != i])
        
        #loader_j = DataLoader(Subset(train_dataset, idx[j]), batch_size=64, shuffle=True)

        # Avaliar a rede i nos dados da rede j antes do treinamento
        #loss_j, error_rate_j, accuracy_j = evaluate_network(net1, loader_j, device)
        #print(f"Pre-training Evaluation: Network {i} on Network {j}'s data - Loss: {loss_j:.4f}, Error Rate: {error_rate_j:.2f}, Accuracy: {accuracy_j:.2f}")
      
        part_loader = DataLoader(Subset(train_dataset, idx[j]), batch_size=64, shuffle=True)

        #print("Treinando a rede "+str(i)+" com dados da rede "+str(j))
...
def train_ring(nets, n_redes, n_epochs, train_dataset, idx, device):
    # Definindo otimizadores para cada rede
    optimizers = []
    criterion = nn.CrossEntropyLoss()
    for i in range(n_redes):
    ...
    # Realizar o treinamento ao longo do anel
    for i in range(n_redes):
        current_node = ring_order[i]
        next_node = ring_order[(i + 1) % n_redes]  # Próximo nó no anel (retorna ao início no último passo)

        net_current = nets[current_node]
        optimizer_current = optimizers[current_node]
        net_current.train()

        # Dados do próximo nó no anel
        part_loader = DataLoader(Subset(train_dataset, idx[next_node]), batch_size=64, shuffle=True)

        # Treinar o nó atual com os dados do próximo nó no anel
        for epoch in range(n_epochs):
            for batch_idx, (data, target) in enumerate(part_loader):
                data, target = data.to(device), target.to(device)
...
def eval_clone(net_clone, test_loader, device):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            #rint(target.ravel(),pred_clone.ravel())
...
def load_nets(dataset, device, n_redes, n_epocas, dirichlet_alpha, epoca, mode, versao, load_idx, versao_idx):
    ...
    idx = []
    if load_idx:
        config_filename = f"cifar10_train_dataset_config_of_10_Epochs_10_0.1_dirichlet_a_versao.pkl"
...
columns = ["dataset", "versao", "accs"]

data_to_save = []


for dataset in ["cifar10"]:
    for versao in ["a","b","c","d","e"]:
...
# Definição do dispositivo a ser usado (GPU se disponível)
        ...
        for i in range(1, 201):
            ...
            if len(nets_tmp) == N_redes:
                ...
                if mode == "ring":
                    nets = train_ring(nets, N_redes, N_epocas, train_dataset, idx, device)
                else:
                    nets = train_random(nets, N_redes, N_epocas, train_dataset, idx, device)
        ...
        data_to_save.append([dataset, versao, accs])
        df_data = pd.DataFrame(data_to_save, columns=columns)
        now = datetime.datetime.now()
        df_data.to_csv(f"outputs_training_{dataset}_{versao}_{now}.csv", index=False) 
